  ...
  return (
    <aside className="sticky top-24 overflow-hidden rounded-[2rem] bg-white shadow-sm ring-1 ring-slate-200/80">
      <div className="border-b border-slate-100 px-6 py-5">
        <p className="text-lg font-semibold text-slate-950">Tóm tắt thanh toán</p>
        <p className="mt-1 text-sm text-slate-500">Rà soát nhanh sản phẩm và tổng hóa đơn trước khi xác nhận.</p>
      </div>
        ...
        {items.map((item, index) => (
          <div key={item.id || index} className="flex gap-3 rounded-2xl bg-slate-50 p-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-xs text-slate-500">{item.variantName || "Mặc định"}</p>
      ...
      <div className="space-y-3 border-t border-slate-100 px-6 py-5 text-sm">
        ...
        <div className="flex items-center justify-between text-slate-500">
          ...
          <span className="font-medium text-slate-900">{formatCurrency(shipping || 0)}</span>
        </div>
        <div className="flex items-center justify-between text-slate-500">
          <span>VAT 10%</span>
        ...
        <div className="border-t border-slate-100 pt-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xl font-semibold text-[#ee4d2d]">{formatCurrency(total)}</span>
          </div>
        </div>
      </div>
    </aside>
