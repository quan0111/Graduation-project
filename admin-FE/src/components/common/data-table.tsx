...
export function DataTable<T extends { id?: number | string }>({
  ...
  const handleExport = (rows: T[]) => {
    const exportData = rows.map((row: any) => {
      const obj: Record<string, unknown> = {};

      columns.forEach((col) => {
        obj[col.label] = col.exportValue ? col.exportValue(row) : row[col.key as string];
      });
  ...
  return (
    <>
      ...
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button size="sm" onClick={() => setExportConfirmOpen(true)} disabled={!data.length}>
          Export Excel
        </Button>
      </CardHeader>
      ...
      <Dialog open={exportConfirmOpen} onOpenChange={setExportConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Xác nhận xuất Excel</DialogTitle>
            <DialogDescription>
              Bạn sắp xuất {exportRows ? "toàn bộ dữ liệu theo bộ lọc hiện tại" : `${data.length} bản ghi`} từ bảng {title || "dữ liệu"} ra file Excel.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setExportConfirmOpen(false)}>
              Hủy
            </Button>
            <Button type="button" onClick={handleConfirmExport} disabled={exportPending}>
              Xác nhận xuất
            </Button>
          </DialogFooter>
        </DialogContent>
